...
def spray(
    ...
    hashes: str | None = typer.Option(None, "--hashes", "-H", help="NTLM hashes (LMHASH:NTHASH)", rich_help_panel="Authentication"),
    aes_key: str | None = typer.Option(None, "--aes-key", help="AES key for Kerberos (128 or 256 bits)", rich_help_panel="Authentication"),
    ...
    if username:
        try:
            credentials = _build_credentials(username, domain, password, hashes, aes_key)
        except ValueError as e:
            logger.error(str(e))
            raise typer.Exit(code=1) from None
...
def _validate_inputs(
    ...
    if username is not None and password is None and hashes is None and (not use_kerberos or aes_key is None):
        raise ConfigurationError(
            f"--password or --hashes{' or --aes-key' if use_kerberos else ''} is required for authentication"
        )

    # Check that only one secret is provided
    secrets = [s for s in (password, hashes, aes_key) if s is not None]
    if len(secrets) > 1:
        raise ConfigurationError("Only one secret can be provided (password, hashes, or aes-key)")
...
def _build_credentials(
    ...
    nt_hash = None
    if hashes:
        # Extract NT hash from LMHASH:NTHASH format
        parts = hashes.split(':')
        if len(parts) == 2:
            nt_hash = parts[1]
        else:
